<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Authentication Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used during authentication for various
    | messages that we need to display to the user. You are free to modify
    | these language lines according to your application's requirements.
    |
    */

    "delete_item" => "حذف العنصر",  

"show_all" => "عرض الكل",  

"cancel" => "إلغاء",  

"no_result" => "لا توجد نتائج",  

"loading" => "جاري التحميل",  

"list" => "قائمة",  

"catgeories" => "الفئات",  

 "accountants" => "المحاسبون",
    "historys" => "التواريخ",
    "show_all" => "عرض الكل",
    "cancel" => "إلغاء",
    "no_result" => "لا توجد نتائج",
    "task" => "مهمة",
    "action" => "إجراء",
    "received" => "تم الاستلام",
    "loginImage" => "صورة تسجيل الدخزل",
    "has" => "لديه",
    "changed_successfully" => "تم التغيير بنجاح",
    "updated_successfully" => "تم التحديث بنجاح",
    "Dashboard" => "لوحة القيادة",
    "Register" => "تسجيل",
    "Name" => "الاسم",
    "Password" => "كلمة المرور",
    "Email" => "البريد الإلكتروني",
    "Login" => "تسجيل الدخول",
    "Confirm" => "تأكيد",
    "Logout" => "تسجيل الخروج",

"dark" => "داكن",  
"newsletters" => "البريد الموحد",  

"newsletterEmail" => "البريد الإلكتروني للنشرة الإخبارية",  

"Dashboard" => "لوحة التحكم",  
'profile' => 'الملف الشخصي',

"Password" => "كلمة المرور",  
'Notifications' => 'الإشعارات',
'DeliveryProcessing' => 'جاري المعالجة',
'OrderComplete' => 'تم الطلب',
'TicketsGenerated' => 'تم إنشاء التذاكر',
'DeliveryComplete' => 'اكتمل التسليم',
'CheckAllNotifications' => 'عرض جميع الإشعارات',

'Users' => 'المستخدمون',
'Orders' => 'الطلبات',
'Settings' => 'الإعدادات',
'LogOut' => 'تسجيل الخروج',
'SupportTickets' => 'تذاكر الدعم',

"Confirm" => "تأكيد",  

"Email" => "البريد الإلكتروني",  

"Register" => "تسجيل",  

"Name" => "الاسم",  

    'vaccancies'=>'العمل المتاح',
    'complains'=>'الشكاوي',
    'home'  => 'الرئيسية',
    'about'  => 'من نحن',
    'title'  => 'العنوان',
    'ar'  => 'العربية',
    'en'  => 'الانجلبزبة',
    'description'  => 'الوصف',
    'image'  => 'الصورة',
    'add_new'  => 'اضافة جديد',
    'save'  => 'حفظ',
    'cancel'  => 'الغاء',
    'subtitle'  => 'العتFوان الفرعي',
    'identifier'  => 'المعرف',
    'edit'  => 'تعديل',
    'show'  => 'عرض',

    'portfolios'  => 'سابقة الاعمال',
    'users'  => 'المستخدمون',
    'admins'  => 'المسؤولون',
    'pages'  => 'الصفحات',
    'services'  => 'الخدمات',
    'testimonials'  => 'اراؤ العملاء',
    'processes'  => 'الخطوات',
    'faqs'  => 'الاستفسارات',
    'counters'  => 'العدادات',
    'contacts'  => 'وسائل الاتصال',
    'videos'  => 'أشرطة الفيديو',
    'partners'  => 'الشركاء',
    'teams'  => 'فريق العمل',
    'messages'  => 'جهات الاتصال',
    'logout'  => 'تسجيل الخروج',
    'login'  => 'تسجيل الدخول',
    'show'  => 'عرض',
    'create'  => 'اضافة',
    'edit'  => 'تعديل',
    'delete'  => 'مسح',
    "projects" => "المشاريع",

    "fees" => "الرسوم",

"show_all" => "عرض الكل",

"cancel" => "إلغاء",

"no_result" => "لا يوجد نتائج",

"amount" => "المبلغ",

"note" => "ملاحظة",

"cost" => "التكلفة",

"rest" => "الباقي",

"specifications" => "المواصفات",

"Dashboard" => "لوحة التحكم",

"Password" => "كلمة المرور",

"Confirm" => "تأكيد",

"Email" => "البريد الإلكتروني",

"Login" => "تسجيل الدخول",

"Register" => "تسجيل",

"Name" => "الاسم",

"Logout" => "تسجيل الخروج",

"tasks" => "المهام",
"finishedTasks" => "المهام المنجزة",
"show_all" => "عرض الكل",
"cancel" => "إلغاء",
"no_result" => "لا توجد نتائج",
"repeat" => "تكرار",
"status" => "الحالة",
"yes" => "نعم",
"no" => "لا",
"employees" => "الموظفون",
"assign_employee" => "تعيين موظف",
"delete_tasks" => "حذف مهام",
"select" => "اختر",
"project" => "مشروع",
"specifications" => "المواصفات",
"employee" => "الموظف",
"salary" => "الراتب",
"Dashboard" => "لوحة التحكم",
"Password" => "كلمة المرور",
"Confirm" => "تأكيد",
"Email" => "البريد الإلكتروني",
"Login" => "تسجيل الدخول",
"Register" => "تسجيل",
"Name" => "الاسم",
"Logout" => "تسجيل الخروج",


    "permission" => "الإذن",
    "setting" => "الجلسة",
    "id" => "بطاقة التعريف",
    "model" => "النموذج",
    "partner" => "الشريك",
    "video" => "الفيديو",
    "service" => "الخدمة",
    "testimonial" => "راي العميل",
    "process" => "الخطوة",
    "partner" => "الشريك",
    "team" => "العضو",
    "counter" => "العداد",
    "contact" => "وسيلة الاتصال",
    "role" => "الدور",
    "page" => "الصفحة",
    "tokenable" => "الرمز",
    "gallery" => "صالة العرض",
    "faq" => "الاستفسار",
    "product" => "المنتج",
    "batch" => "حزمة",
    "fileable" => "قابلة للمرشح",
    "exception" => "الاستثناء",
    "payload" => "الحمولة",
    "count" => "العدد",
    "map" => "الخريطة",
    "twitter" => "تويتر",
    "queue" => "الطابور",
    "tiktok" => "تيك توك",
    "phone1" => "الهاتف 1",
    "email1" => "البريد الالكتروني1",
    "abilities" => "القدرات",
    "phone3" => "الهاتف3",
    "description" => "الوصف",
    "detail" => "التفاصيل",
    "appointment1" => "الموعد 1",
    "meta_data" => "البيانات الوصفية",
    "facebook" => "فيسبوك",
    "youtube_link" => "رابط يوتيوب",
    "connection" => "الاتصال",
    "youtube" => "اليوتيوب",
    "instgram" => "الانستجرام",
    "phone2" => "الهاتف2",
    "email2" => "البريد الإلكتروني 2",
    "address" => "عنوان",
    "copyright" => "حقوق الملكية",
    "created_at" => "أنشئت في",
    "email_verified_at" => "eMail_verified_at",
    "failed_at" => "فشل",
    "updated_at" => "تم التحديث في",
    "last_used_at" => "last_used_at",
    "phone" => "الهاتف",
    "logo" => "الشعار",
    "white_logo" => "الشعارالابيض",
    "uuid" => "uuid",
    "email" => "البريد إلكتروني",
    "identifier" => "المعرف",
    "password" => "كلمة المرور",
    "token" => "الرمز",
    "tokenable_type" => "tokenable_type",
    "url" => "الرابط",
    "title" => "العنوان",
    "fileable_type" => "fileable_type",
    "image" => "الصورة",
    "locale" => "اللغة",
    // "message" => "الرسالة",
    "tab" => "فاتورة غير مدفوعة",
    "model_type" => "نوع النموذج",
    "name" => "الاسم",
    "remember_token" => "تذكر _token",
    "guard_name" => "guard_name",
    "subtitle" => "العنوان الفرعي",
    "migration" => "الهجرة",
    "controls" => "التحكم",
    "faqs" => "الاستفسارات",
    "dashboard" => "لوحة التحكم",
    "userreg" => "المستخدمين المسجلين",
    "moreinfo" => "المزيد",
    "add" => "اضافة جديد",
    "choose_file" => "اختر ملف",
    "upload_file" => "تحميل الملف",
    "confirm_password" => "تاكيد كلمة المرور",
    "new" => "جديد",
    "errors" => "الاخطاء",
    "admin" => "المسئول",
    "sent_successfully" => "تم الارسال!",
    "failed" => "حدث خطأ ما!",
    "roles" => "الادوار",
    "role_mangement" => "ادارة الادوار",
    "settings" => "الاعدادات",
    "logo" => "اللوجو",
    "tab" => "التاب",

    "choose" => "اختر",
    "file" => "ملف",

    "instagram" => "انستجرام",
    "linkedin" => "لينكدان",



    'opening_hour' => 'توقيت العمل ',
    'hotline' => 'اتصل بنا',
    'follow_us_on' => ' : تواصل معنا عبر',
    'home' => 'الرئيسية',
    'services' => 'خدماتنا',
    'search' => 'بحث',
    'about' => 'من نحن',
    'contact_us' => 'تواصل معنا',
    'get_started' => 'ابدأ معنا',
    'all_services' => 'خدماتنا',
    'address' => 'العنوان',
    'team' => 'الفريق',
    'portfolio' => 'صفحة الشخصية',
    'partner' => 'الشريك',
    'your_email' => 'بريدك الالكترونى',
    'fullname' => 'اسمك بالكامل',
    'email_address' => 'بريدك الالكترونى',
    'message' => 'رسالتك',
    'submit' => 'قدم الان',
    'useful_link' => 'روابط تساعدك',
    'all_rights' => 'جميع الحقوق محفوظه @ ',
    'select_icon' => 'اختر ايقونة',
    'type' => 'النوع',
    'our_services'=>'خدماتنا',
    'images'=>'الصور',
    'free_consulting'=>'خدمات استشارية',
    'selectOne'=>'يرجى تحديد',
    'social'=>'وسائل التواصل الاجتماعي',
    'whatsapp'=>'واتساب',
    'icon'=>'أيقونة',
    'subject'=>'موضوع',
    'reply'=>'رد',
    'visit_our_website'=>'قم بزيارة موقعنا',
    'subscribe_now'=>'اشترك فى قائمتنا البريدية',
    'show_more'=>'اعرض اكثر',
    'show_less'=>'اعرض اقل',
    'created_successfully'=>'تم الانشاء بنجاح',
    'update_successfully'=>'تم التعديل بنجاح',
    'deleted_successfully'=>'تم الحذف بنجاح',
    'something_wrong'=>'يوجد خطأ ما !!',
    'show_all'=>'عرض الكل',
    'credentials_error'=>'هذه المدخلات لا تتناسب مع قاعدة بياناتنا',
    'dselectAll'=>'الغاء تحديد الكل',
    'selectAll'=>'تحديد الكل',

    'delete_confirm'=>'تاكيد الحذف',
    'delete_message'=>'هل انت متاكد انك تريد حذف',
    "search" => "بحث",
    "products" => "المنتجات",
    "categories" => "الاقسام",
    "show_all" => "عرض الكل",
    "cancel" => "يلغي",
    "no_result" => "لا توجد نتيجه",
    "send_message" => "أرسل رسالة",
    "user" => "مستخدم",
    "serviceRequest" => "طلب خدمة",
    "our_portfolios" => "معرضنا",
    "Dashboard" => "لوحة التحكم",
    "Password" => "الرقم السرى",
    "Confirm" => "تأكيد",
    "Email" => "بريد إلكتروني",
    "Login" => "تسجيل الدخول",
    "Register" => "يسجل",
    "Name" => "اسم",
    "Logout" => "تسجيل خروج",


    // New keys added on 2025-09-21 14:01:34
    'choose_files' => 'اختر الملفات',
    'exams' => 'الامتحانات',
    'lessons' => 'الدروس',
    'courses' => 'الدورات',
    'materials' => 'المواد التعليمية',
    'price' => 'السعر',
    'discount' => 'الخصم',
    'start_date' => 'تاريخ البدء',
    'end_date' => 'تاريخ الانتهاء',
    'active' => 'نشط',
    'level' => 'المستوى',
    'language' => 'اللغة',
    'introVideo' => 'فيديو تقديمي',
    'curriculum' => 'المنهج الدراسي',
    'inactive' => 'غير نشط',
    'duration_hours' => 'مدة بالساعات',
    'question' => 'السؤال',
    'enter_question' => 'أدخل السؤال',
    'answer_option' => 'خيار الإجابة',
    'addAnswer' => 'أضف إجابة',
    'correct_answer' => 'الإجابة الصحيحة',
    'select_correct_answer' => 'اختر الإجابة الصحيحة',
    'addQuestion' => 'أضف سؤال',
    'lesson' => 'الدرس',
    'course' => 'الدورة',
    'answers' => 'الإجابات',
    'duration' => 'المدة',
    'order' => 'الطلب',
    'is_free' => 'مجاني',
    'free_lesson' => 'درس مجاني',
    'no_files' => 'لا توجد ملفات',
    'newsletter' => 'النشرة الإخبارية',
    'back' => 'عودة',
    'serviceRequests' => 'طلبات الخدمة',
    'appointment' => 'موعد',
    'is_active' => 'نشط',
    'not_found' => 'غير موجود',
    'browser_not_support_video' => 'المتصفح لا يدعم تشغيل الفيديو',
    'no_video_available' => 'لا يوجد فيديو متاح',
    'xxx' => 'XXX',
    'account_deleted' => 'تم حذف الحساب',
    'account_must_be_verified' => 'يجب التحقق من الحساب',
    'invalid_credentials' => 'بيانات اعتماد غير صالحة',
    'verification_code_resent' => 'تم إعادة إرسال رمز التحقق',
    'account_created_verify_email' => 'تم إنشاء الحساب، يرجى التحقق من البريد الإلكتروني',
    'account_created_successfully' => 'تم إنشاء الحساب بنجاح',
    'invalid_otp' => 'رمز التحقق غير صالح',
    'account_verified_success' => 'تم التحقق من الحساب بنجاح',
    'customer_signed_out' => 'تم تسجيل خروج العميل',
    'no_user_authenticated' => 'لا يوجد مستخدم مسجل دخوله',
    'out_of_stock_items' => 'عناصر نفدت من المخزون',
    'finished_offer_items' => 'العروض انتهت',
    'noEnoughBalance' => 'الرصيد غير كافٍ',
    'want_to_change_order' => 'هل تريد تغيير الطلب؟',
    'want_to_enter' => 'هل تريد الدخول؟',
    'dont_want_to_continue' => 'لا أريد المتابعة',
    'find_cheaper' => 'إيجاد أرخص',
    'dont_want_to_buy' => 'لا أريد الشراء',
    'others' => 'أخرى',
    'order_cannot_be_cancelled' => 'لا يمكن إلغاء الطلب',
    'order_cannot_be_cancelled_due_to_payment' => 'لا يمكن إلغاء الطلب بسبب الدفع',
    'profile_updated' => 'تم تحديث الملف الشخصي',
    'password_changed' => 'تم تغيير كلمة المرور',
    'exam' => 'الامتحان',
    
];